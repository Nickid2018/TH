...
import com.github.isam.ISAM;
import com.github.isam.sound.SoundSystem;
import com.github.isam.util.ObjectUtils;
...
public class CrashReport {
    ...
    private CrashReportSession system;
    ...
    private void fillSystemDetails() {
        system = new CrashReportSession("System & Runtime Details");
        sessions.add(system);
        system.addDetail("ISAM version", Constants.VERSION_IN_STRING);
        system.addDetail("Operating System",
                String.format("%s (%s) version %s", System.getProperty("os.name"),
                        System.getProperty("os.arch"), System.getProperty("os.version")));
        system.addDetail("Java Version",
                String.format("%s, %s", System.getProperty("java.version"), System.getProperty("java.vendor")));
        system.addDetail("Java VM Version",
                String.format("%s (%s), %s", System.getProperty("java.vm.name"),
                        System.getProperty("java.vm.info"), System.getProperty("java.vm.vendor")));
        system.addDetail("OpenGL Version",
                ISAM.getInstance().getRenderer() == null? "<Unknown>" : ISAM.getInstance().getRenderer().getGLVersionString());
        system.addDetail("OpenAL Version", SoundSystem.getAlVersion());
        system.addDetail("Backend Library", String.format("LWJGL version %s", Version.getVersion()));
        system.addDetail("Memory", () -> {
            ...
            return String.format("%d bytes (%d MB) / %d bytes (%d MB) up to %d bytes (%d MB)", l2, l5, l1, l4, l, l3);
        });
        ...
        HardwareAbstractionLayer hardware = ObjectUtils.getOrNull(() -> new SystemInfo().getHardware());
        ...
        for (int i = 0; i < gcards.size(); i++) {
            ...
            system.addDetail("Graphics Card #" + i, () -> {
                ...
                return String.format("Name=%s; Vendor=%s; VRAM=%.2fMB; DeviceId=%s; VersionInfo=%s",
                        ...
                        card.getVRam() / 1048576.0F, card.getDeviceId(), card.getVersionInfo());
            });
        }
        ...
        for (int i = 0; i < phymems.size(); i++) {
            ...
            system.addDetail("Memory Slot #" + i, () -> {
                ...
                return String.format("Capacity=%.2fMB; ClockSpeed=%.2fGHz; Type=%s",
                        mem.getCapacity() / 1048576.0F,
                        mem.getClockSpeed() / 1.0E9F, mem.getMemoryType());
            });
        }
        system.addDetail("Virtual Memory", () -> {
            ...
            return String.format("Max=%.2fMB; Used=%.2fMB; Swap Total=%.2fMB; Swap Used=%.2fMB",
                    mem.getVirtualMax() / 1048576.0F,
                    mem.getVirtualInUse() / 1048576.0F,
                    mem.getSwapTotal() / 1048576.0F,
                    mem.getSwapUsed() / 1048576.0F);
        });
    }
}
